...
>
    {[
        " Event Video",
        " Live Streaming",
        ...(selectedItem?.type_id !== 0 ? [" Gen AI View"] : []),
    ].map((label, index) => (
        <Tab
            key={index}
            label={label}
                                                ...
                                                <Box sx={{ display: 'flex', flexDirection: 'column' }}>
                                                    <CardContent sx={{ flex: '1 0 auto', fontSize: "10px" }}>
                                                        <Typography color="black" sx={{ fontSize: "14px" }}>
                                                            {selectedItem.event_type_name}
                                                        </Typography>
                                                        <Typography variant='body-2' sx={{ color: "red", textAlign: "start", py: "0px", paddingTop: "5px" }}>
                                                            {selectedItem?.is_in_property === 1 ? "Still on property" : selectedItem?.is_in_property === 0 ? "Not on property" : ""}
                                                        </Typography>
                                                        {/* <Typography sx={{ fontSize: "12px" }}>
                                                            Age: {selectedItem?.plate || ''}
                                                        </Typography>
                                                        <Typography sx={{ fontSize: "12px" }}>
                                                            Gender: {selectedItem?.record?.car_type || ''}
                                                        </Typography>
                                                        <Typography sx={{ fontSize: "12px" }}>
                                                            Clothing: {selectedItem?.record?.color || ''}
                                                        </Typography> */}
                                                        <Typography sx={{ fontSize: "12px" }}>
                                                            Event Captured: {selectedItem?.create_time || ''}
                                                        </Typography>
                                                        {/* Resolved/Unresolved Select */}
                                                        {selectedItem?.is_resolved === 0 ? (
