...
const CameraMap = ({ data, Cameramappolepopupdata }) => {
  ...
  useEffect(() => {
    if (CameraPolemapData) {
      const validLocations = CameraPolemapData.filter(location => {
        return !isNaN(parseFloat(location.location_lat)) && !isNaN(parseFloat(location.location_lng));
      });

      if (validLocations.length > 0) {
        const totalLat = validLocations.reduce((acc, loc) => acc + parseFloat(loc.location_lat), 0);
        const totalLng = validLocations.reduce((acc, loc) => acc + parseFloat(loc.location_lng), 0);
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (response.data.code === 200) {
          ...
          dispatch(fetchCameraMapPoleDataSuccess(data));
