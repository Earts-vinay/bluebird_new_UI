...
// Thunk to Get a Latest Total Alerts 
export const latestTotalAlerts = createAsyncThunk(
  'Alert/latestTotalAlerts',
  async ({ propertyId, startDate, endDate, type, typeId }, { getState, rejectWithValue }) => {
    console.log("typpp", type);
...
// Thunk to Get a Latest Total Alerts 
export const previousTotalAlerts = createAsyncThunk(
  'Alert/previousTotalAlerts',
  async ({ propertyId, startDate, endDate, type, typeId }, { getState, rejectWithValue }) => {
    console.log("typpp", type);
...
const AlertSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      //latest total alerts
      .addCase(latestTotalAlerts.pending, (state) => {
        state.loading = true;
        state.error = null;
      })
      .addCase(latestTotalAlerts.fulfilled, (state, action) => {
        console.log("aaaaaaaaaaa", action.payload);
        state.loading = false;
        state.latestTotalAlerts = action.payload;
      })
      .addCase(latestTotalAlerts.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload || 'Error fetching data';
      })

      //previous total alerts 
      .addCase(previousTotalAlerts.pending, (state) => {
        state.loading = true;
        state.error = null;
      })
      .addCase(previousTotalAlerts.fulfilled, (state, action) => {
        console.log("aaaaaaaaaaa", action.payload);
        state.loading = false;
        state.previousTotalAlerts = action.payload;
      })
      .addCase(previousTotalAlerts.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload || 'Error fetching data';
      })
