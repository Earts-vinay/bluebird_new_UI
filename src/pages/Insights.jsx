...
//         const currentMonthEnd = today.clone(); // current date
...
//         // Cards: Last month
//         previousStartDate = lastMonthStart.format("YYYY-MM-DD");
//         previousEndDate = lastMonthEnd.format("YYYY-MM-DD");
      
//         // Charts: This month (from start of this month to today)
//         latestStartDate = currentMonthStart.format("YYYY-MM-DD");
//         latestEndDate = currentMonthEnd.format("YYYY-MM-DD");
//       }
...
const Insights = () => {
  ...
  const calculateDateRange = (range) => {
    ...
    return { previousStartDate, previousEndDate, latestStartDate, latestEndDate };
  };

  const updateDateRange = (range) => {
    // Calculate new dates immediately
    const newDateRange = calculateDateRange(range);
    
    // Update dateRange state synchronously to prevent flashing
    ...
    setLoading(true);
    setTimeout(() => {
      // Persist to localStorage after loading
      localStorage.setItem("selectedRange", range);
              ...
              <RangePicker
                value={
                  dateRange.latestStartDate && dateRange.latestEndDate
                    ? [dayjs(dateRange.latestStartDate), dayjs(dateRange.latestEndDate)]
                    : undefined
                }
                disabledDate={(current) => current && current > dayjs()}
                onChange={(dates) => {
